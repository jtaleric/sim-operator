...
IMG ?= quay.io/jtaleric/sim-operator:latest
...
.PHONY: docker-build
docker-build: ## Build docker image with the manager (uses Containerfile).
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	# copy Containerfile and insert --platform=${BUILDPLATFORM} into Dockerfile.cross for multi-arch build
	sed -e '1 s/\(^FROM\)/FROM --platform=\$$\{BUILDPLATFORM\}/; t' -e ' 1,// s//FROM --platform=\$$\{BUILDPLATFORM\}/' Containerfile > Dockerfile.cross
...
deploy: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	...
	oc new-project sim-operator-system
...
deploy-samples: ## Deploy sample ScaleLoadConfig resources
	oc apply -f config/samples/

.PHONY: undeploy-samples
undeploy-samples: ## Remove sample ScaleLoadConfig resources
	oc delete --ignore-not-found=$(ignore-not-found) -f config/samples/
