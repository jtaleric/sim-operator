...
type DeletionManager struct {
	...
	mutex      sync.RWMutex
	pending    map[string][]client.Object // resourceType -> objects waiting for deletion
...
func NewDeletionManager(reconciler *ScaleLoadConfigReconciler) *DeletionManager {
	return &DeletionManager{
		...
		pending:    make(map[string][]client.Object),
...
func (dm *DeletionManager) GetDeletionStatus() scalev1.ResourceDeletionStatus {
	...
	status := scalev1.ResourceDeletionStatus{
		PendingDeletions:  make(map[string]int32),
	...
	// Copy pending deletions
	for resourceType, objects := range dm.pending {
		status.PendingDeletions[resourceType] = int32(len(objects))
	}
